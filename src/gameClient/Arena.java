package gameClient;

import api.directed_weighted_graph;
import api.edge_data;
import api.geo_location;
import api.node_data;
import gameClient.util.Point3D;
import gameClient.util.Range;
import gameClient.util.Range2D;
import gameClient.util.Range2Range;
import org.json.JSONArray;
import org.json.JSONException;
import org.json.JSONObject;

import java.util.ArrayList;
import java.util.Iterator;
import java.util.List;

/**
 * This class represents a multi Agents Arena which move on a graph - grabs Pokemons and avoid the Zombies.
 */
public class Arena {
    public static final double EPS1 = 0.001, EPS2 = EPS1 * EPS1;
    private float time;
    private directed_weighted_graph _gg;
    private List<CL_Agent> _agents;
    private List<CL_Pokemon> _pokemons;
    private List<String> _info;
    private static Point3D MIN = new Point3D(0, 100, 0);
    private static Point3D MAX = new Point3D(0, 100, 0);

    /**
     * this method is a constructor for a new arena
     */
    public Arena() {
        _info = new ArrayList<String>();
    }

    /**
     * this method is constructor for a new arena
     *
     * @param g ->graph of the arena
     * @param p ->Pokemon's of the game
     * @param r -> agent of the game
     */
    private Arena(directed_weighted_graph g, List<CL_Agent> r, List<CL_Pokemon> p) {
        _gg = g;
        this.setAgents(r);
        this.setPokemons(p);
    }

    /**
     * this method return the time of the game
     *
     * @return time
     */
    public float getTime() {
        return this.time;
    }

    /**
     * this method set a new time to the arena
     *
     * @param time
     */
    public void setTime(float time) {
        this.time = time;
    }

    /**
     * this time set a new pokemon list to the arena
     *
     * @param f->pokemon list
     */
    public void setPokemons(List<CL_Pokemon> f) {
        this._pokemons = f;
    }

    /**
     * this method set a new agent list to this arena
     *
     * @param f ->agent list
     */
    public void setAgents(List<CL_Agent> f) {
        this._agents = f;
    }

    /**
     * this method set a new graph to this arena
     *
     * @param g ->graph
     */
    public void setGraph(directed_weighted_graph g) {
        this._gg = g;
    }

    /**
     * this method return the agent list of this arena
     *
     * @return agent list
     */
    public List<CL_Agent> getAgents() {
        return _agents;
    }

    /**
     * this method return the Pokemon's list of this arena
     *
     * @return Pokemon's list
     */
    public List<CL_Pokemon> getPokemons() {
        return _pokemons;
    }

    /**
     * this method return the graph of this arena
     *
     * @return graph of this arena
     */
    public directed_weighted_graph getGraph() {
        return _gg;
    }

    /**
     * this method return the information of this arena
     *
     * @return information of this arena
     */

    public List<String> get_info() {
        return _info;
    }

    /**
     * this method set a new information to this arena
     *
     * @param _info->info for set
     */
    public void set_info(List<String> _info) {
        this._info = _info;
    }

    /**
     * this method get a json string and the graph game and return the graph agent list
     *
     * @param gg ->graph game
     * @param aa ->json string
     * @return agent list
     */
    public static List<CL_Agent> getAgents(String aa, directed_weighted_graph gg) {
        ArrayList<CL_Agent> ans = new ArrayList<CL_Agent>();
        try {
            JSONObject ttt = new JSONObject(aa);
            JSONArray ags = ttt.getJSONArray("Agents");
            for (int i = 0; i < ags.length(); i++) {
                CL_Agent c = new CL_Agent(gg, 0);
                c.update(ags.get(i).toString());
                ans.add(c);
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
        return ans;
    }

    /**
     * this method get an json string and conver it to pokemon list of the game
     * this method does not up data the edges of the Pokemon's
     *
     * @param fs ->json string
     * @return list of Pokemon's
     */
    public static ArrayList<CL_Pokemon> json2Pokemons(String fs) {
        ArrayList<CL_Pokemon> ans = new ArrayList<CL_Pokemon>();
        try {
            JSONObject ttt = new JSONObject(fs);
            JSONArray ags = ttt.getJSONArray("Pokemons");
            for (int i = 0; i < ags.length(); i++) {
                JSONObject pp = ags.getJSONObject(i);
                JSONObject pk = pp.getJSONObject("Pokemon");
                int t = pk.getInt("type");
                double v = pk.getDouble("value");
                String p = pk.getString("pos");
                CL_Pokemon f = new CL_Pokemon(new Point3D(p), t, v, null);
                ans.add(f);
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
        return ans;
    }

    /**
     * this method get a pokemon and the graph game and up data the edge of the pokemon
     * by using isOnEdge method
     *
     * @param g  ->graph of the game
     * @param fr ->pokemon for up data edge
     */
    public static void updateEdge(CL_Pokemon fr, directed_weighted_graph g) {
        Iterator<node_data> itr = g.getV().iterator();
        while (itr.hasNext()) {
            node_data v = itr.next();
            Iterator<edge_data> iter = g.getE(v.getKey()).iterator();
            while (iter.hasNext()) {
                edge_data e = iter.next();
                boolean f = isOnEdge(fr.getLocation(), e, fr.getType(), g);
                if (f) {
                    fr.set_edge(e);
                }
            }
        }
    }

    /**
     * this method get Location of: pokemon,src edge,dest edge.
     *
     * @param p    ->pokemon location
     * @param dest ->dest location
     * @param src  ->src location
     * @return true this pokemon on this edge else return false
     */
    private static boolean isOnEdge(geo_location p, geo_location src, geo_location dest) {

        boolean ans = false;
        double dist = src.distance(dest);
        double d1 = src.distance(p) + p.distance(dest);
        if (dist > d1 - EPS2 * EPS2) {
            ans = true;
        }
        return ans;
    }

    /**
     * this method get the position of the pokemon and the source edge
     * the destination edge and the graph game
     * and convert the src to Location of node src
     * and dest to Location dest
     * and check if the pokemon on giving edge by using private method is on edge
     *
     * @param g->graph   game
     * @param p->pokemon location
     * @param s->edge    source
     * @param d->edge    destination
     * @return true if the pokemon on the edge else return false
     */
    private static boolean isOnEdge(geo_location p, int s, int d, directed_weighted_graph g) {
        geo_location src = g.getNode(s).getLocation();
        geo_location dest = g.getNode(d).getLocation();
        return isOnEdge(p, src, dest);
    }

    /**
     * this method get the location of the pokemon an edge from the graph the type of the pokemon
     * and the graph and check if the edge on the graph by using private method isOnEdge
     *
     * @param p    ->pokemon location
     * @param g    ->graph of the game
     * @param e    ->edge
     * @param type ->type of the pokemon
     * @return true if the pokemon on the edge else return false
     */
    private static boolean isOnEdge(geo_location p, edge_data e, int type, directed_weighted_graph g) {
        int src = g.getNode(e.getSrc()).getKey();
        int dest = g.getNode(e.getDest()).getKey();
        if (type < 0 && dest > src) {
            return false;
        }
        if (type > 0 && src > dest) {
            return false;
        }
        return isOnEdge(p, src, dest, g);
    }

    /**
     * this method go all over the graph and take its node and convert it
     * to range2D object.
     * that make max and min of axis y as the highest and the lowest vertices accordingly
     * and make the max and the min of axis x as the most right and most left vertices accordingly
     *
     * @param g ->graph of the game
     * @return the range2D
     */
    private static Range2D GraphRange(directed_weighted_graph g) {
        Iterator<node_data> itr = g.getV().iterator();
        double x0 = 0, x1 = 0, y0 = 0, y1 = 0;
        boolean first = true;
        while (itr.hasNext()) {
            geo_location p = itr.next().getLocation();
            if (first) {
                x0 = p.x();
                x1 = x0;
                y0 = p.y();
                y1 = y0;
                first = false;
            } else {
                if (p.x() < x0) {
                    x0 = p.x();
                }
                if (p.x() > x1) {
                    x1 = p.x();
                }
                if (p.y() < y0) {
                    y0 = p.y();
                }
                if (p.y() > y1) {
                    y1 = p.y();
                }
            }
        }
        Range xr = new Range(x0, x1);
        Range yr = new Range(y0, y1);
        return new Range2D(xr, yr);
    }

    /**
     * this method adapt the world's coordinates to the wanted frame's coordinates
     *
     * @param g     ->graph of the game
     * @param frame
     */
    public static Range2Range w2f(directed_weighted_graph g, Range2D frame) {
        Range2D world = GraphRange(g);
        Range2Range ans = new Range2Range(world, frame);
        return ans;
    }

}